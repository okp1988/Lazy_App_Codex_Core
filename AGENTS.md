...
- Before committing or pushing, include new files `CountdownProgressControl.cs`, `SkipPickerControl.cs`, `RunSetControl.cs`, `RunSetControl.Designer.cs`, and `RunSetControl.resx`; otherwise a pull on another machine will not have the current run-set UI.
