...
- `Form1.cs` owns the main window, global hotkey routing, two independent run-slot states, offset selection, fixed one-set/two-set window sizing, taskbar overlay icons, and opening the config editor.
...
- The current PC layout uses compact fixed sizes: one-set window `500 x 264`, two-set window `984 x 264`, run-set content column `322`, action column `150`, and `RunSetControl` `472 x 216`. These values removed the previous large PC whitespace. If adjusting a laptop layout, preserve the PC profile or add an explicit alternate profile instead of silently changing these values.
...
- When tuning laptop design later, compare against a PC screenshot before and after. Do not break the PC compact layout while adjusting laptop spacing. If laptop needs different dimensions, add a guarded layout profile keyed by DPI/screen/font context, leaving the PC constants unchanged.
