...
- The current compact layout fixes the usable client area, not the outer window chrome: one-set client `484 x 224`, two-set client `968 x 224`, run-set content column `322`, action column `150`, and `RunSetControl` `472 x 216`. `Form1` derives the outer fixed size with `SizeFromClientSize(...)` so Windows DPI/title-bar differences can add a few outer pixels without squeezing the app contents. If adjusting a laptop layout, preserve this client-size profile or add an explicit alternate profile instead of silently changing these values.
...
- `RunSetControl` action controls use six fixed `34px` rows plus a `12px` blank bottom spacer. Do not let `TableLayoutPanel` leftover height stretch the Pair / Connect button; add or adjust an explicit spacer row when bottom whitespace changes.
- When tuning laptop design later, compare against a PC screenshot before and after. Do not break the compact client-size layout while adjusting laptop spacing. If laptop needs different dimensions, add a guarded layout profile keyed by DPI/screen/font context, leaving the current client-size constants unchanged.
- Before committing or pushing, include new files `RunSetControl.cs`, `RunSetControl.Designer.cs`, and `RunSetControl.resx`; otherwise a pull on another machine will not have the run-set control.
